# Patch final-fix.py with enhanced regex for all audio/image formats
f = open("/tmp/final-fix.py")
c = f.read()
f.close()

# Replace find_audio function entirely
old_find = '''def find_audio(scene_id, pnum):
    """智能查找 part N 的音频"""
    ...
    for af in mp3_files:
        afl = af.lower()
        # Match patterns like: part1-zh.mp3, s14-part1-zh.mp3, part01-zh.mp3
        pattern_zh = r'(.*?)-?part%d-?zh\\.mp3$' % pnum
        pattern_en = r'(.*?)-?part%d-?en\\.mp3$' % pnum
        
        if re.search(pattern_zh, afl):
            found_zh = af
        if re.search(pattern_en, afl):
            found_en = af
...
new_find = '''def find_audio(scene_id, pnum):
...
# Also fix image parsing to handle s06p1.jpg format
...
f.write(c)
f.close()
print("patched find_audio + image parsing")
